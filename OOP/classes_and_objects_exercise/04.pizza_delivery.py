class PizzaDelivery:
    ...
    def remove_ingredient(self, ingredient: str, quantity: int, price_per_quantity: float):
        ...
        if not self.ingredients.get(ingredient, 0):
    ...
    def make_order(self):
        self.ordered = True
        ingredients_quantity_str = ', '.join(f"{ingredient}: {quantity}"
                                             for ingredient, quantity in self.ingredients.items())

        return (f"You've ordered pizza {self.name} prepared with {ingredients_quantity_str} "
