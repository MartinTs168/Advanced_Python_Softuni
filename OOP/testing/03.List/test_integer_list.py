...
class TestIntegerList(unittest.TestCase):
    ...
    def test_add_non_integer_element_to_list_raises_value_error(self):
        with self.assertRaises(ValueError) as ve:
            self.i_list.add(5.5)

        self.assertEqual("Element is not Integer", str(ve.exception))

    def test_add_integer_adds_integer_to_list(self):
    ...
    def test_insert_with_out_of_range_index_raises_index_error(self):
        with self.assertRaises(IndexError) as ie:
            self.i_list.insert(1000, 2)

        self.assertEqual("Index is out of range", str(ie.exception))

    def test_insert_with_valid_index_and_non_integer_raises_value_error(self):
        with self.assertRaises(ValueError) as ve:
            self.i_list.insert(2, "doi")

        self.assertEqual("Element is not Integer", str(ve.exception))

    def test_insert__integer_on_valid_index(self):
